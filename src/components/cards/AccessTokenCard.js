...
const AccessTokenCard = ({ token, onEdit, onDelete, showActions }) => {
    const title = token.access_key_name || (token.access_key_id ? `Access Key #${token.access_key_id}` : 'Access Key');
    return (
        <div className="bg-black3 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between">
            ...
            {showActions && (
                <div className="flex items-center space-x-3 mt-4 sm:mt-0">
                    ...
                    <button
                        onClick={() => onDelete && onDelete(token.access_key_id)}
